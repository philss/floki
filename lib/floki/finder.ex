defmodule Floki.Finder do
  ...
  def find_and_transform({name, attrs, rest}, transformation) do
    {new_name, new_attrs} = transformation.({name, attrs})
    new_rest = Enum.map(rest, fn
      html_tree -> find_and_transform(html_tree, transformation)
    end)
    {new_name, new_attrs, new_rest}
  end

  def find_and_transform(x, transformation), do: x
